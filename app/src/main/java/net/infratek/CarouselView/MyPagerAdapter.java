...
public class MyPagerAdapter extends FragmentPagerAdapter implements
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (positionOffset >= 0f && positionOffset <= 1f) {
            ...
            if (nextnext != null) {
                nextnext.setScaleBoth(MainActivity.SMALL_SCALE);
                ViewHelper.setAlpha(nextnext, minAlpha);
                //ViewHelper.setRotationY(nextnext, 0);
            }

            if (prevprev != null) {
                prevprev.setScaleBoth(MainActivity.SMALL_SCALE);
                ViewHelper.setAlpha(prevprev, minAlpha);
            }

            if (cur != null) {
                float curScale = MainActivity.BIG_SCALE - MainActivity.DIFF_SCALE * positionOffset;
                Log.d(TAG, ">> curScale " + curScale);
                cur.setScaleBoth(curScale);
